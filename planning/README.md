...
Specs, plans, and change history for `faststream-redis-timers`. The living
truth about *what the system does now* lives in
...
committed copy to drift. Run `just check-planning` to validate every change
and decision.

## Other

- **[`architecture/`](../architecture/)** at the repo root — the living
  capability truth and the promotion target on every ship. Capability files are
  authored over time as the system grows.
- **[`architecture/glossary.md`](../architecture/glossary.md)** — the ubiquitous
  language: the domain terms code, specs, and capability pages share.
- **[changes/](changes/)** — the per-change files (one change file from the
  design template, or one from the change template).
- **[decisions/](decisions/)** — design decisions taken (and alternatives
  rejected), each with a revisit trigger; indexed by `just index`.
- **[releases/](releases/)** — per-release user-facing notes; the release
  workflow uses `releases/<tag>.md` verbatim as the GitHub Release body.
